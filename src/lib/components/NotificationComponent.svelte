<script>
    import {notification, updateNotification} from '$lib/store.js'
...
<aside class="k-toast animate-bounce" hidden={$notification.status === "hidden"}>
    <section>
        <div class="flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800"
             role="alert">
            {@html $notification.status in typeMap ? typeMap[$notification.status]: typeMap.success}
