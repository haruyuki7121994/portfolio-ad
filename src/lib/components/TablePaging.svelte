...
<div class="flex flex-col items-center px-5 py-5 bg-white xs:flex-row xs:justify-between">
    <div class="flex items-center">
        ...
        {#each {length: meta.totalPages} as _, page}
            <button type="button"
                    class={`w-full px-4 py-2 text-base ${currentPage === page + 1 ? "text-indigo-500" : "text-gray-600"} bg-white border hover:bg-gray-100`}
                    on:click={() => updatePage(page)}
            >
                {page + 1}
            </button>
        {/each}
